...
export const CASE_STATUSES = [
  ...
  'in_progress', 'under_review', 'pending_court', 'closed', 'resolved',
];
...
export class AdminCaseTableComponent implements OnInit, OnDestroy {
  ...
  private static readonly STORAGE_KEY_COLUMNS = 'admin_case_table_columns';
  private static readonly STORAGE_KEY_DENSITY = 'admin_case_table_density';
  ...
  loadCases(): void {
    ...
    this.adminService.getAllCasesTable(params).pipe(
  ...
  onColumnsChange(columns: string[]): void {
    ...
    localStorage.setItem(AdminCaseTableComponent.STORAGE_KEY_COLUMNS, JSON.stringify(merged));
  }

  onDensityChange(value: TableDensity): void {
    this.density.set(value);
    localStorage.setItem(AdminCaseTableComponent.STORAGE_KEY_DENSITY, value);
  }
  ...
  private loadPreferences(): void {
    try {
      const savedCols = localStorage.getItem(AdminCaseTableComponent.STORAGE_KEY_COLUMNS);
      if (savedCols) {
    ...
    const savedDensity = localStorage.getItem(AdminCaseTableComponent.STORAGE_KEY_DENSITY);
