...
@Component({
  ...
  template: `
    <div class="profile-page">
      ...
      <mat-card class="profile-card">
        <mat-card-header>
          <mat-card-title>{{ 'OPR.PROFILE_INFO' | translate }}</mat-card-title>
        ...
        <mat-card-content>
          @if (editingProfile()) {
            ...
            @if (currentUser()) {
              <div class="info-grid">
                <div><span class="lbl">{{ 'OPR.FULL_NAME' | translate }}</span><span>{{ currentUser()!.name }}</span></div>
                <div><span class="lbl">{{ 'OPR.EMAIL' | translate }}</span><span>{{ currentUser()!.email }}</span></div>
                @if (currentUser()!.phone) {
                  <div><span class="lbl">{{ 'OPR.PHONE' | translate }}</span><span>{{ currentUser()!.phone }}</span></div>
                }
              </div>
            }
          }
      ...
      <mat-card class="profile-card">
        <mat-card-header>
          <mat-card-title>{{ 'OPR.CHANGE_PASSWORD' | translate }}</mat-card-title>
          <div class="card-header-action">
            <button mat-button (click)="toggleEditPassword()">
              {{ editingPassword() ? ('COMMON.CANCEL' | translate) : ('OPR.CHANGE' | translate) }}
            </button>
          </div>
        </mat-card-header>
        @if (editingPassword()) {
          <mat-card-content>
            <form [formGroup]="passwordForm" (ngSubmit)="savePassword()" class="edit-form">
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>{{ 'OPR.CURRENT_PASSWORD' | translate }}</mat-label>
                <input matInput formControlName="currentPassword" type="password" />
              </mat-form-field>
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>{{ 'OPR.NEW_PASSWORD' | translate }}</mat-label>
                <input matInput formControlName="newPassword" type="password" />
              </mat-form-field>
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>{{ 'OPR.CONFIRM_PASSWORD' | translate }}</mat-label>
                <input matInput formControlName="confirmPassword" type="password" />
              </mat-form-field>
              <button mat-raised-button color="primary" type="submit" [disabled]="savingPassword()">
                @if (savingPassword()) { Updating... } @else { {{ 'OPR.UPDATE_PASSWORD' | translate }} }
              </button>
            </form>
          </mat-card-content>
        }
  ...
  styles: [`
    ...
    .avatar-icon { font-size: 56px; width: 56px; height: 56px; color: #bbb; }
    .avatar-overlay {
...
export class OperatorProfileComponent implements OnInit {
  ...
  fullName = computed(() => this.currentUser()?.name || 'Operator');
