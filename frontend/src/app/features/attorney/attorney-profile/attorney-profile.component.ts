...
@Component({
  ...
  template: `
    <div class="profile-page">
      ...
      <mat-card class="profile-card">
        <mat-card-header>
          <mat-card-title>{{ 'ATT.PROFILE_INFO' | translate }}</mat-card-title>
          <div class="card-header-action">
            <button mat-button (click)="toggleEditProfile()">
              {{ editingProfile() ? ('COMMON.CANCEL' | translate) : ('COMMON.EDIT' | translate) }}
            </button>
          </div>
        </mat-card-header>
        <mat-card-content>
          @if (editingProfile()) {
            <form [formGroup]="profileForm" (ngSubmit)="saveProfile()" class="edit-form">
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>{{ 'ATT.FIRST_NAME' | translate }}</mat-label>
                <input matInput formControlName="firstName" type="text" autocomplete="given-name" />
              </mat-form-field>
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>{{ 'ATT.LAST_NAME' | translate }}</mat-label>
                <input matInput formControlName="lastName" type="text" autocomplete="family-name" />
              </mat-form-field>
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>{{ 'ATT.EMAIL' | translate }}</mat-label>
                <input matInput formControlName="email" type="email" inputmode="email" autocomplete="email" />
              </mat-form-field>
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>{{ 'ATT.PHONE' | translate }}</mat-label>
                <input matInput formControlName="phone" type="tel" inputmode="tel" autocomplete="tel" />
              </mat-form-field>
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>{{ 'ATT.BAR_NUMBER' | translate }}</mat-label>
                <input matInput formControlName="barNumber" type="text" />
              </mat-form-field>
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>{{ 'ATT.FIRM_NAME' | translate }}</mat-label>
                <input matInput formControlName="firmName" type="text" />
              </mat-form-field>
              <button mat-raised-button color="primary" type="submit" [disabled]="savingProfile()">
                @if (savingProfile()) { Saving... } @else { {{ 'COMMON.SAVE' | translate }} }
              </button>
            </form>
          } @else {
            @if (currentUser()) {
              <div class="info-grid">
                <div><span class="lbl">{{ 'ATT.FULL_NAME' | translate }}</span><span>{{ currentUser()!.name }}</span></div>
                <div><span class="lbl">{{ 'ATT.EMAIL' | translate }}</span><span>{{ currentUser()!.email }}</span></div>
                @if (currentUser()!.phone) {
                  <div><span class="lbl">{{ 'ATT.PHONE' | translate }}</span><span>{{ currentUser()!.phone }}</span></div>
                }
                <div><span class="lbl">{{ 'ATT.BAR_NUMBER' | translate }}</span><span>{{ barNumber() }}</span></div>
                <div><span class="lbl">{{ 'ATT.FIRM_NAME' | translate }}</span><span>{{ firmName() }}</span></div>
              </div>
            }
          }
        </mat-card-content>
      ...
      <mat-card class="profile-card">
        <mat-card-header>
          <mat-card-title>{{ 'ATT.CHANGE_PASSWORD' | translate }}</mat-card-title>
          <div class="card-header-action">
            <button mat-button (click)="toggleEditPassword()">
              {{ editingPassword() ? ('COMMON.CANCEL' | translate) : ('ATT.CHANGE' | translate) }}
            </button>
          </div>
        </mat-card-header>
        @if (editingPassword()) {
          <mat-card-content>
            <form [formGroup]="passwordForm" (ngSubmit)="savePassword()" class="edit-form">
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>{{ 'ATT.CURRENT_PASSWORD' | translate }}</mat-label>
                <input matInput formControlName="currentPassword" type="password" />
              </mat-form-field>
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>{{ 'ATT.NEW_PASSWORD' | translate }}</mat-label>
                <input matInput formControlName="newPassword" type="password" />
              </mat-form-field>
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>{{ 'ATT.CONFIRM_PASSWORD' | translate }}</mat-label>
                <input matInput formControlName="confirmPassword" type="password" />
              </mat-form-field>
              <button mat-raised-button color="primary" type="submit" [disabled]="savingPassword()">
                @if (savingPassword()) { Updating... } @else { {{ 'ATT.UPDATE_PASSWORD' | translate }} }
              </button>
            </form>
          </mat-card-content>
        }
  ...
  styles: [`
    ...
    .avatar-button:hover { border-color: #1976d2; }
...
export class AttorneyProfileComponent implements OnInit {
  ...
  fullName = computed(() => this.currentUser()?.name || 'Attorney');
  avatarUrl = computed(() => this.currentUser()?.avatar_url || '');
  barNumber = computed(() => this.currentUser()?.barNumber || 'N/A');
  firmName = computed(() => 'N/A');
